...
export function convertDropboxToDirectLink(url) {
  ...
  console.log("[PatreonArchiver Cloud] Converting Dropbox URL:", url);
  let clean = url.trim();
...
export async function resolveGoogleDriveDirectLink(url) {
  console.log("[PatreonArchiver Cloud] Resolving Google Drive URL via Service Worker:", url);
  ...
  try {
    ...
    if (bgResult && bgResult.ok && bgResult.directUrl) {
      console.log("[PatreonArchiver Cloud] Service Worker resolved Google Drive direct URL:", bgResult.directUrl);
      return bgResult;
    }
  } catch (err) {
...
export async function resolveMediaFireDirectLink(url) {
  console.log("[PatreonArchiver Cloud] Resolving MediaFire URL via Service Worker:", url);
  try {
    const bgResult = await new Promise((resolve) => {
...
export async function resolveWeTransferDirectLink(url) {
  console.log("[PatreonArchiver Cloud] Resolving WeTransfer URL via Service Worker:", url);
  try {
    const bgResult = await new Promise((resolve) => {
...
export async function resolveDirectDownloadUrlWithName(url) {
  ...
  console.log(`[PatreonArchiver Cloud] resolveDirectDownloadUrlWithName: provider="${provider}" input="${url}" -> resolved="${result.url}"${result.url === url ? " (UNCHANGED - resolution likely failed, see warnings above)" : ""}`);
  return result;
}
