...
// v3: Store "logs" fuer den persistenten Log-Ringpuffer (siehe lib/appLog.js).
// Die Upgrade-Routine legt jeden Store nur an, wenn er fehlt - eine Erhoehung
// der Version laesst bestehende Creator-/Post-/Settings-Daten also unangetastet.
const DB_VERSION = 3;
  ...
  dbPromise = new Promise((resolve, reject) => {
    ...
    req.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains("logs")) {
        // autoIncrement: der Schluessel ist gleichzeitig die Reihenfolge - damit
        // laesst sich der aelteste Eintrag ohne Sortieren finden (Ringpuffer).
        db.createObjectStore("logs", { keyPath: "seq", autoIncrement: true });
      }
    };
...
// ---------- Log-Ringpuffer (siehe lib/appLog.js) ----------
//
// Bewusst hier und nicht in appLog.js: db.js ist die EINZIGE Stelle, die die
// IndexedDB-Verbindung kennt (Background-Worker UND Dashboard importieren sie
// gemeinsam). appLog.js bleibt dadurch frei von Datenbank-Details.
...
// Behaelt nur die neuesten `max` Eintraege. Loescht ueber einen Cursor von vorne
// (kleinster Schluessel = aeltester Eintrag), statt alles zu laden.
export async function trimLogs(max) {
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const t = db.transaction("logs", "readwrite");
    const store = t.objectStore("logs");
    ...
    countReq.onsuccess = () => {
      ...
      cursorReq.onsuccess = () => {
        ...
        if (!cursor || toDelete <= 0) { resolve(deleted); return; }
        cursor.delete();
        toDelete--;
        cursor.continue();
      };
...
export async function getAllLogs() {
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const req = tx(db, "logs", "readonly").getAll();
    req.onsuccess = () => resolve(req.result || []);
    req.onerror = () => reject(req.error);
  });
}
...
export async function clearLogs() {
  const db = await openDB();
  return new Promise((resolve, reject) => {
    const req = tx(db, "logs", "readwrite").clear();
    req.onsuccess = () => resolve();
    req.onerror = () => reject(req.error);
  });
}
