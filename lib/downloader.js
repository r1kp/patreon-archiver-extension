...
import { getFilenameFromCloudUrl, resolveDirectDownloadUrl, resolveDirectDownloadUrlWithName, unsupportedCloudProvider, isCloudFolderUrl, extractGoogleDriveFileId } from "./cloudDownloader.js";
import { createPool } from "./concurrencyPool.js";
import { logMilestone, logWarn } from "./appLog.js";

export function canonicalCloudKey(url) {
  if (!url) return null;
  const driveId = extractGoogleDriveFileId(url);
  if (driveId) return `gdrive:${driveId}`;
  const megaMatch = url.match(/mega\.(?:nz|io)\/(?:folder|file)\/([a-zA-Z0-9_-]+)/i) || url.match(/mega\.(?:nz|io)\/#(?:F|N)!([a-zA-Z0-9_-]+)/i);
  if (megaMatch) return `mega:${megaMatch[1]}`;
  const dropMatch = url.match(/dropbox\.com\/scl\/fo\/([a-zA-Z0-9_-]+)/i) || url.match(/dropbox\.com\/s\/([a-zA-Z0-9_-]+)/i);
  if (dropMatch) return `dropbox:${dropMatch[1]}`;
  return null;
}
...
export async function downloadItems(items, settings, options = {}) {
  ...
  const inFlightCloudDownloads = new Map(); // canonicalCloudKey -> Promise<string>
  ...
  function thumbnailSignalForPost(post, passedSignal) {
    if (passedSignal && passedSignal !== cancelSignal) return passedSignal;
    const tUrl = post.thumbnail?.url || `${post.id}::thumbnail`;
    return itemCombinedSignal(post, { role: "thumbnail", kind: "thumbnail", url: tUrl });
  }

  function ensureThumbnail(creatorName, post, passedSignal) {
    const postId = String(post.id);
    ...
    const effectiveSignal = thumbnailSignalForPost(post, passedSignal);
    const p = applicable
      ? imagePool.run(() => doThumbnail(creatorName, post, effectiveSignal, ctx))
  ...
  async function doThumbnail(creatorName, post, cancelSignal, ctx) {
    ...
    if (cancelSignal?.cancelled) {
      reportStep(`${post.title} · Thumbnail (cancelled)`, true, {
        weight: thumbWeight,
        postId: post.id,
        itemKind: "thumbnail",
        url: post.thumbnail?.url || `${post.id}::thumbnail`,
        status: "cancelled"
      });
  ...
  function videoSignalForPost(post, passedSignal) {
    if (passedSignal && passedSignal !== cancelSignal) return passedSignal;
    const vUrl = post.video?.url || `${post.id}::video`;
    const vKind = post.video?.type === "native" ? "video" : "embed";
    return itemCombinedSignal(post, { role: "video", kind: vKind, url: vUrl });
  }

  function ensureVideo(creatorName, post, passedSignal) {
    const postId = String(post.id);
    if (videoPromises.has(postId)) return videoPromises.get(postId);
    const ctx = getPostContext(post);
    const applicable = !!post.video && (ctx.isFullPost || ctx.wantsVideo);
    const effectiveSignal = videoSignalForPost(post, passedSignal);
    const p = applicable
      ? videoPool.run(() => doVideo(creatorName, post, effectiveSignal, ctx))
  ...
  async function doVideo(creatorName, post, cancelSignal, ctx) {
    ...
    if (cancelSignal?.cancelled) {
      const vKind = post.video.type === "native" ? "video" : "embed";
      reportStep(`${post.title} · Video (cancelled)`, true, {
        isVideoPhase: false,
        weight: videoWeight,
        postId: post.id,
        itemKind: vKind,
        url: post.video.url || `${post.id}::video`,
        status: "cancelled"
      });
      return { videoSkipped: true, cancelled: true };
    }
      ...
      if (bridgeConnected && pingResult.ytdlpFound) {
        ...
        if (existingFile) {
          ...
          function applyProgress(line) {
            ...
            const feedResult = feedProgress(line);
            const { pct, phase, phaseLabel, speed, totalBytes } = feedResult;
            if (phase === "merging" && !mergeTickInterval) {
            ...
            if (line && line.includes("has already been downloaded")) {
              ...
              if (!stepCounted) {
                reportStep(`${post.title} · Video (skipped)`, true, { isVideoPhase: false, weight: videoWeight, postId: post.id, itemKind: "embed", url: post.video.url, status: "done" });
                stepCounted = true;
              }
            } else if (onStep) {
              let labelText = `${post.title} · Video`;
              if (phase === "scanning") {
                labelText += " (Scanning...)";
              } else if (pct > 0) {
                labelText += ` (${Math.round(pct)}%${speed ? ' · ' + speed : ''})`;
              }
              reportStep(labelText, false, {
                isVideoPhase: true,
                currentStepWeight: videoWeight,
                postId: post.id,
                itemKind: "embed",
                url: post.video.url,
                ...
                phase: phase,
                speed: speed,
                status: phase === "scanning" ? "scanning" : "active"
              });
            }
          }
          ...
          try {
            ...
            console.error(`[PatreonArchiver:Downloader Video Error] [${post.title}]`, err);
  ...
  function triggerPostExtras(creatorName, post, cancelSignal, allTasksRef) {
    ...
    allTasksRef.push(ensureDescription(creatorName, post, itemCombinedSignal(post, { role: "description", kind: "description", url: `${post.id}::desc` })));
    allTasksRef.push(ensureComments(creatorName, post, itemCombinedSignal(post, { role: "comments", kind: "comments", url: `${post.id}::comments` })));
    allTasksRef.push(ensureThumbnail(creatorName, post, thumbnailSignalForPost(post, cancelSignal)));
    allTasksRef.push(ensureVideo(creatorName, post, videoSignalForPost(post, cancelSignal)));
  }
    ...
    try {
      ...
      if (file.isWebsite || file.isCloudLink || file.isExternalLink) {
        ...
        if (unsupportedProvider) {
          logWarn(`Unsupported cloud provider "${unsupportedProvider}" - skipped: ${file.url}`);
          results[i] = {
            url: file.url,
            ok: false,
            unsupportedProvider,
        ...
        logMilestone(`Cloud download -> ${providerSubfolder} | folder="${cloudFolderName}" | url=${file.url}`);
        ...
        const cloudKey = canonicalCloudKey(file.url);
        try {
          if (cloudKey && inFlightCloudDownloads.has(cloudKey)) {
            logMilestone(`Cloud deduplication: waiting on active cloud download for ${cloudKey}`);
            try {
              const prevPath = await inFlightCloudDownloads.get(cloudKey);
              if (prevPath && (await checkFileExistsViaBridge(prevPath))) {
                actualBridgePath = prevPath;
              }
            } catch (prevErr) {
          ...
          const downloadTask = (async () => {
            ...
            return typeof r === "string" && r ? r : targetAbsPath;
          })();

          if (cloudKey) inFlightCloudDownloads.set(cloudKey, downloadTask);
          actualBridgePath = await downloadTask;
        } catch (bridgeErr) {
