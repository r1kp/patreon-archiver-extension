  ...
  return new Promise((resolve, reject) => {
    ...
    port.onMessage.addListener((msg) => {
      if (msg.type === "progress") {
        ...
        console.error("[PatreonArchiver:NativeHost Error]", msg.message);
        settled = true;
        cleanup();
    ...
    port.onDisconnect.addListener(() => {
      ...
      if (!settled) {
        ...
        console.error("[PatreonArchiver:NativeHost Disconnected]", err || "Host disconnected before completion");
        reject(
          new Error(
            err
