  ...
  return new Promise((resolve, reject) => {
    ...
    port.onMessage.addListener((msg) => {
      if (msg.type === "progress") {
        console.log("[PA ytdlp progress]", msg.line);
        ...
        if (msg.line.includes("No supported JavaScript runtime could be found") ||
            ...
            msg.line.includes("HTTP Error 403") ||
            msg.line.includes("Forbidden")) {
        ...
        console.log("[PA ytdlp] keepalive received", new Date().toISOString());
...
export function pingYtDlpHost(forceVersionCheck = false) {
  return new Promise((resolve) => {
    ...
    port.onMessage.addListener((msg) => {
      if (msg.type === "pong") {
        answered = true;
        resolve({ ok: true, ytdlpFound: msg.ytdlpFound, version: msg.version || "1.0.0" });
        port.disconnect();
      }
    });
    port.onDisconnect.addListener(() => {
