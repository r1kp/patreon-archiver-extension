  ...
  function feed(line) {
    ...
    if (currentPct >= 0 && downloadPhase !== "merger") {
      if (isAudioOnly) {
        ...
        const calc = currentPct * 0.8;
        if (calc > maxPct) maxPct = calc;
      } else if (downloadPhase === "audio") {
        const calc = 80 + currentPct * 0.15;
        if (calc > maxPct) maxPct = calc;
      }
    }

    if (downloadPhase === "merger") {
      // ffmpeg liefert beim Muxen normalerweise keine eigenen %-Zeilen mehr.
      // Statt bei 95% einzufrieren, langsam Richtung 99% treiben (nie 100% -
      // das setzt ausschließlich der Aufrufer, sobald "done" wirklich kommt).
      const elapsed = mergerStartedAt ? Date.now() - mergerStartedAt : 0;
      const trickle = 95 + Math.min(4, elapsed / 1500); // +1% pro 1.5s, Deckel 99%
      if (trickle > maxPct) maxPct = trickle;
    }

    let effectivePhase = downloadPhase;
    if (downloadPhase === "video" && maxPct === 0 && currentPct < 0) {
      effectivePhase = "scanning";
    }

    const phaseLabel = effectivePhase === "scanning"
      ? "Scanning..."
      : effectivePhase === "audio"
      ? (isAudioOnly ? "Audio" : "Audio Track")
      : effectivePhase === "merger"
      ? "Merging"
      : "Video Track";
