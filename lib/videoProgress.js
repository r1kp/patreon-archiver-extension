...
export function createVideoProgressTracker(isAudioOnly, videoTitle = "") {
  ...
  function feed(line) {
    ...
    const tag = videoTitle ? ` [${videoTitle.length > 20 ? videoTitle.substring(0, 18) + '…' : videoTitle}]` : "";
    console.log(`[PatreonArchiver:Video${tag}] [${effPhase}] ${effPct.toFixed(1)}% | speed: ${effSpeed || '-'} | total: ${totalBytes ? Math.round(totalBytes / 1024 / 1024) + 'MB' : '-'} | raw: ${line.trim()}`);
